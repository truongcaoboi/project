    ...
    export default {
        ...
        methods:{
            getUserDetail: async function(){
                let res = await RestFul.getWithToken("/api/manage/admin/getById/"+store.user.id);
                if(res != null){
            ...
            submitForm(formName) {
                ...
                this.$refs[formName].validate(async function(valid){
                    if (valid) {
                        let res = await RestFul.putWithToken("/api/manage/admin/update", me.params);
                        if(res != null){
                            if(res.statusCode == 200){
                        ...
                        if(me.paramChangePass.oldPassword == store.user.password){
                            if(me.paramChangePass.oldPassword != me.paramChangePass.password){
                                if(me.paramChangePass.password == me.paramChangePass.repassword){
                                    ...
                                    let res = await RestFul.putWithToken("/api/manage/admin/changePassword", user);
                                    if(res != null){
                                        if(res.statusCode == 200){
                                            me.$message.success(res.message);
                                            me.$router.push("/login");
